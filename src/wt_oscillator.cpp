...
double WTOscillator::DoSquareWaveCore(double *read_idx, double wt_inc) {
  ...
  if (wt_inc >= 0) {
    if (pwidx >= WT_LENGTH) pwidx = pwidx - WT_LENGTH;
  } else {
    if (pwidx < 0) pwidx = pwidx + WT_LENGTH;
  }
...
inline void wt_check_wrap_index(double *index) {
  while (*index < 0.0) *index += WT_LENGTH;
  while (*index >= WT_LENGTH) *index -= WT_LENGTH;
}
