...
void MiniSynth::Update()
{
    ...
    m_global_synth_params.eg1_params.legato_mode =
        (bool)m_settings.m_legato_mode;
    ...
    if (m_settings.m_note_number_to_decay_scaling == 1)
        modmatrix.EnableMatrixRow(SOURCE_MIDI_NOTE_NUM,
...
void MiniSynth::IncrementVoiceTimestamps()
{
    for (int i = 0; i < MAX_VOICES; i++)
    {
        if (voices_[i]->m_note_on)
            voices_[i]->m_timestamp++;
    }
}
...
std::shared_ptr<MiniSynthVoice> MiniSynth::GetOldestVoice()
{
    ...
    for (int i = 0; i < MAX_VOICES; i++)
    {
        ...
        {
            if (voices_[i]->m_note_on &&
                (int)voices_[i]->m_timestamp > timestamp)
            {
...
std::shared_ptr<MiniSynthVoice> MiniSynth::GetOldestVoiceWithNote(int midi_note)
{
    int timestamp = -1;
    std::shared_ptr<MiniSynthVoice> found_voice = NULL;
    for (auto v : voices_)
