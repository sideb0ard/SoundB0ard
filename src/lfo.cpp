...
double LFO::DoOscillate(double *quad_phase_output) {
  ...
  if (modmatrix) {
    ...
    modmatrix->sources[m_mod_dest_output2] = qp_out * m_amplitude * m_amp_mod;
  }

  if (quad_phase_output) *quad_phase_output = qp_out * m_amplitude * m_amp_mod;

  return out * m_amplitude * m_amp_mod;
}

void LFO::StartOscillator() {
  if (m_lfo_mode == LFOSYNC || m_lfo_mode == LFOSHOT) Reset();
