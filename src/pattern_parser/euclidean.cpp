...
cat_vector_vector(std::vector<std::vector<int>> &front_vec,
                  std::vector<std::vector<int>> &back_vec)
{
    std::vector<std::vector<int>> ret;
    for (auto i : front_vec)
        ret.push_back(i);
    for (auto i : back_vec)
        ret.push_back(i);
...
std::string generate_euclidean_string(int num_hits, int len_sequence)
{
    ...
    for (int i = 0; i < num_hits; i++)
        front_vec.push_back(std::vector{1});
...
std::vector<std::vector<int>>
euclidean_recursor(std::vector<std::vector<int>> front_vec,
                   std::vector<std::vector<int>> back_vec)
{
    if (back_vec.size() < 2)
    {
