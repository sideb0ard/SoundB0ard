...
void euclidean_status(void *self, wchar_t *wstring)
{
    ...
    swprintf(wstring, MAX_PS_STRING_SZ,
             ...
             "wee EUCLIDEAN hits:%d steps:%d", e->num_hits, e->num_steps);
}
                                       ...
                                       int *count, int *remaindrrr)
{
    ...
    else
    {
        for (int i = 0; i < count[level]; i++)
        {
            build_euclidean_pattern_string(level - 1, bitmap_string,
                                        count, remaindrrr);
        }
        if (remaindrrr[level] != 0)
        {
            build_euclidean_pattern_string(level - 2, bitmap_string,
                                        count, remaindrrr);
        }
    }
}
