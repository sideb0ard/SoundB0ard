...
void granulator_status(void *self, char *status_string)
{
    ...
    snprintf(status_string, MAX_PS_STRING_SZ,
             "numgrains: %d grainlen: %d fudge: %d wetmx: %f read_pos: %d locked: %s bufferfull: %s env: %s",
             g->num_grains, g->grain_duration_ms, g->fudge_factor,
             g->wet_mix, g->read_pos,
             g->locked ? "true" : "false",
...
double granulator_process_audio(void *self, double input)
{
    ...
    for (int i = 0; i < g->num_grains; i++) {
        ...
        if (grain_idx != -1) {
            ...
            //int modified_idx = g->m_delay.m_read_index + grain_idx + (i*samplesize_between_grains);
...
void granulator_set_read_pos(granulator *g, int percent)
{
    ...
    if (percent < 100) {
        ...
        //printf("POS is %d delay_in_samp: %f, pct: %d\n", pos, g->m_delay.m_delay_in_samples, percent);
        g->read_pos = pos;
    }
}
