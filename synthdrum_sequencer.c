...
synthdrum_sequencer *new_synthdrum_seq(int drumtype)
{
    ...
    sds->m_eg1.m_attack_time_msec = 0;
    sds->m_eg1.m_decay_time_msec = 35;
    sds->m_eg1.m_release_time_msec = 200;

    envelope_generator_init(&sds->m_eg2);
    sds->m_eg2.m_attack_time_msec = 0;
    sds->m_eg2.m_decay_time_msec = 0;
    ...
    else if (status == 176) // control
    {
        ...
        case 1:
            if (sds->midi_controller_mode == 0) {
                ...
                scaley_val = scaleybum(0, 127, 2, 200, data2);
        ...
        case 3:
            if (sds->midi_controller_mode == 0) {
                scaley_val = scaleybum(0, 127, 2, 500, data2);
                eg_set_attack_time_msec(&sds->m_eg1, scaley_val);
                printf("ATTACK!! %f\n", scaley_val);
            }
            else {
