...
{
    FILTER_CSEM *filter =
        (FILTER_CSEM *)calloc(1, sizeof(FILTER_CSEM));
...
{
    FILTER_CSEM *self = filter;
    self->bc_filter->m_q_control = (25.0 - 0.5)*(qcontrol - 1.0)/(10.0 - 1.0) + 0.5;
}
...
double csem_gennext(void *filter, double xn)
{
    ...
    if (self->bc_filter->m_type != LPF2 
            && self->bc_filter->m_type != HPF2
            && self->bc_filter->m_type != BPF2
            && self->bc_filter->m_type != BSF2)
        return xn;

    double hpf = self->m_alpha0 * (xn - self->m_rho * self->m_z11 - self->m_z12);
    ...
    double semBSF = self->bc_filter->m_aux_control * hpf + (1.0 - self->bc_filter->m_aux_control) * lpf;
