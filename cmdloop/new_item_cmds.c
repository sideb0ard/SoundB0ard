    ...
    if (strncmp("new", wurds[0], 3) == 0)
    {
                 ...
                 strncmp("ssynth", wurds[1], 6) == 0)
        {
            if (strlen(wurds[2]) != 0)
            {
                ...
                if (sgnum != -1)
                {
                    sound_generator *sg = mixr->sound_generators[sgnum];
        ...
        if (strncmp("markov", wurds[1], 6) == 0)
        {
            ...
            if (strncmp("clap2", wurds[2], 5) == 0)
                mixer_add_markov(mixr, 0);
            else if (strncmp("kick2", wurds[2], 5) == 0)
                mixer_add_markov(mixr, 1);
            else if (strncmp("claps", wurds[2], 5) == 0)
                ...
                mixer_add_markov(mixr, 9);
            else if (strncmp("raggasnare", wurds[2], 10) == 0)
                mixer_add_markov(mixr, 10);
            else
            {
                 ...
                 strncmp("step", wurds[1], 4) == 0)
        {
            ...
            if (strlen(wurds[2]) == 0 || strncmp("synth", wurds[2], 5) == 0)
            {
                ...
                sgnum = add_sound_generator(mixr, (sound_generator *)ds);
            }
            else if (is_valid_file(wurds[2]))
            {
                drumsampler *s = new_drumsampler(wurds[2]);
                sgnum = add_sound_generator(mixr, (sound_generator *)s);
            }
            if (sgnum != -1)
            {
                sound_generator *sg = mixr->sound_generators[sgnum];
