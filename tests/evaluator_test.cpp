...
bool TestNumberObject(std::shared_ptr<object::Object> obj, int64_t expected) {
  ...
  if (io->value_ != expected) {
    std::cerr << "TEST NUMBER OBJECT - val not correct - actual:" << io->value_
...
TEST_F(EvaluatorTest, TestIfElseExpression) {
  ...
  std::vector<std::string> nullTests{"if (false) { 10 }", "if (1 > 2) { 10 }"};
...
TEST_F(EvaluatorTest, TestInBuiltFunctions) {
  ...
  std::vector<TestCaseInt> tests{
      {R"(len(""))", 0}, {R"(len("four"))", 4}, {R"(len("hello wurld"))", 11}};
  for (auto &tt : tests) {
    EXPECT_TRUE(TestNumberObject(TestEval(tt.input), tt.expected));
  }
}
  ...
  std::string input = R"(let two = "two";
{
  ...
  for (auto &e : expected) {
    ...
    if (p == hsh->pairs_.end()) FAIL() << "No pair found for key in pairs...";
