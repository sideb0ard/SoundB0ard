...
//        {R"(push(1, 1))", "argument to `push` must be an array - got
//        INTEGER"},
//    };
//
...
//        {R"(let i = 7; let x = 10; for (i = 5; i > 0; --i) { let x = x + i;
//        puts(x); }; i)",
//         "7"},
//        {R"(let x = 10; for (i = 5; i > 0; --i) { let x = x + i; puts(x); i;
//        }; i;)",
...
//    };
//    std::vector<TestCase> tests{
//        {R"({"foo": 5}["foo"])", 5}, {R"(let key = "foo"; {"foo": 5}[key])",
//        5}, {R"({5: 5}[5])", 5},         {R"({true: 5}[true])", 5},
//        {R"({false: 5}[false])", 5},
//    };
//
//    for (auto &tt : tests)
