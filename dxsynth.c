...
static char *s_dx_dest_names[] = {"DX_DEST_NONE", "DX_DEST_AMP_MOD", "DX_DEST_VIBRATO"};
...
dxsynth *new_dxsynth(void)
{
    ...
    dxsynth_voice_initialize_modmatrix(dx->m_voices[0], &dx->m_global_modmatrix);
...
void dxsynth_update(dxsynth *dx)
{
    ...
    dx->m_global_synth_params.osc1_params.amplitude = calculate_dx_amp(dx->m_settings.m_op1_output_lvl);
    dx->m_global_synth_params.osc2_params.amplitude = calculate_dx_amp(dx->m_settings.m_op2_output_lvl);
    dx->m_global_synth_params.osc3_params.amplitude = calculate_dx_amp(dx->m_settings.m_op3_output_lvl);
    dx->m_global_synth_params.osc4_params.amplitude = calculate_dx_amp(dx->m_settings.m_op4_output_lvl);
    ...
    dx->m_global_synth_params.osc1_params.waveform = dx->m_settings.m_op1_waveform;
    dx->m_global_synth_params.osc2_params.waveform = dx->m_settings.m_op2_waveform;
    dx->m_global_synth_params.osc3_params.waveform = dx->m_settings.m_op3_waveform;
    dx->m_global_synth_params.osc4_params.waveform = dx->m_settings.m_op4_waveform;

    dx->m_global_synth_params.osc1_params.cents = dx->m_settings.m_op1_detune_cents;
    dx->m_global_synth_params.osc2_params.cents = dx->m_settings.m_op2_detune_cents;
    dx->m_global_synth_params.osc3_params.cents = dx->m_settings.m_op3_detune_cents;
    dx->m_global_synth_params.osc4_params.cents = dx->m_settings.m_op4_detune_cents;

    // EG1
    dx->m_global_synth_params.eg1_params.attack_time_msec = dx->m_settings.m_eg1_attack_ms;
    dx->m_global_synth_params.eg1_params.decay_time_msec = dx->m_settings.m_eg1_decay_ms;
    dx->m_global_synth_params.eg1_params.sustain_level = dx->m_settings.m_eg1_sustain_lvl;
    dx->m_global_synth_params.eg1_params.release_time_msec = dx->m_settings.m_eg1_release_ms;
    dx->m_global_synth_params.eg1_params.reset_to_zero = (bool)dx->m_settings.m_reset_to_zero;
    dx->m_global_synth_params.eg1_params.legato_mode = (bool)dx->m_settings.m_legato_mode;

    // EG2
    dx->m_global_synth_params.eg2_params.attack_time_msec = dx->m_settings.m_eg2_attack_ms;
    dx->m_global_synth_params.eg2_params.decay_time_msec = dx->m_settings.m_eg2_decay_ms;
    dx->m_global_synth_params.eg2_params.sustain_level = dx->m_settings.m_eg2_sustain_lvl;
    dx->m_global_synth_params.eg2_params.release_time_msec = dx->m_settings.m_eg2_release_ms;
    dx->m_global_synth_params.eg2_params.reset_to_zero = (bool)dx->m_settings.m_reset_to_zero;
    dx->m_global_synth_params.eg2_params.legato_mode = (bool)dx->m_settings.m_legato_mode;

    // EG3
    dx->m_global_synth_params.eg3_params.attack_time_msec = dx->m_settings.m_eg3_attack_ms;
    dx->m_global_synth_params.eg3_params.decay_time_msec = dx->m_settings.m_eg3_decay_ms;
    dx->m_global_synth_params.eg3_params.sustain_level = dx->m_settings.m_eg3_sustain_lvl;
    dx->m_global_synth_params.eg3_params.release_time_msec = dx->m_settings.m_eg3_release_ms;
    dx->m_global_synth_params.eg3_params.reset_to_zero = (bool)dx->m_settings.m_reset_to_zero;
    dx->m_global_synth_params.eg3_params.legato_mode = (bool)dx->m_settings.m_legato_mode;

    // EG4
    dx->m_global_synth_params.eg4_params.attack_time_msec = dx->m_settings.m_eg4_attack_ms;
    dx->m_global_synth_params.eg4_params.decay_time_msec = dx->m_settings.m_eg4_decay_ms;
    dx->m_global_synth_params.eg4_params.sustain_level = dx->m_settings.m_eg4_sustain_lvl;
    dx->m_global_synth_params.eg4_params.release_time_msec = dx->m_settings.m_eg4_release_ms;
    dx->m_global_synth_params.eg4_params.reset_to_zero = (bool)dx->m_settings.m_reset_to_zero;
    dx->m_global_synth_params.eg4_params.legato_mode = (bool)dx->m_settings.m_legato_mode;

    // LFO1
    dx->m_global_synth_params.lfo1_params.waveform = dx->m_settings.m_lfo1_waveform;
    dx->m_global_synth_params.lfo1_params.osc_fo = dx->m_settings.m_lfo1_rate;

    // DCA
    dx->m_global_synth_params.dca_params.amplitude_db = (bool)dx->m_settings.m_volume_db;

    if (dx->m_settings.m_lfo1_mod_dest1 == DX_LFO_DEST_NONE)
    {
        enable_matrix_row(&dx->m_global_modmatrix, SOURCE_LFO1, DEST_OSC1_OUTPUT_AMP,
                          false);
    ...
    {
        enable_matrix_row(&dx->m_global_modmatrix, SOURCE_LFO1, DEST_OSC2_OUTPUT_AMP,
                          false);
    ...
    {
        enable_matrix_row(&dx->m_global_modmatrix, SOURCE_LFO1, DEST_OSC3_OUTPUT_AMP,
                          false);
    ...
    {
        enable_matrix_row(&dx->m_global_modmatrix, SOURCE_LFO1, DEST_OSC4_OUTPUT_AMP,
                          false);
                             ...
                             unsigned int velocity, bool all_notes_off)
{
    ...
    {
        dxsynth_voice *msv =
            dxsynth_get_oldest_voice_with_note(ms, midinote);
        if (msv)
        {
...
void dxsynth_midi_pitchbend(dxsynth *ms, unsigned int data1,
                              unsigned int data2)
{
...
void dxsynth_status(void *self, wchar_t *status_string)
{
    ...
    swprintf(
        ...
        L"[" WANSI_COLOR_WHITE "DXSYNTH '%s'] - Vol: %.2f Active:%s\n"
        ...
        dx->vol, dx->sound_generator.active ? "true" :" false",
        dx->m_settings.m_lfo1_intensity,
