...
const char *s_event_type[] = {"midi", "32nd", "24th", "16th", "12th",
                              "8th",  "6th",  "4th",  "3rd",  "bar"};
const char *s_timer_type[] = {"every", "over", "for"};
...
void Timer::EventNotify(mixer_timing_info tinfo)
{
    if (!active_)
        return;
    ...
    if (event_type_ == TIME_MIDI_TICK)
        Eval();
    else if (event_type_ == TIME_START_OF_LOOP_TICK && tinfo.is_start_of_loop)
    {
        Eval();
    }
...
//           "z..\"|rand=\"x y z..\"|osc=\"lo hi\">)] Timer \%%s\n");
//}
//
...
//    else
//    {
//        printf("Need a Timer type - 'every', 'over' or 'for'\n");
//        return false;
//    }
//
//    if (a->Timer_type == FOR)
...
void Timer::Status(wchar_t *status_string)
{
    ...
    swprintf(status_string, MAX_STATIC_STRING_SZ,
             WANSI_COLOR_WHITE "%sTimer:" WANSI_COLOR_WHITE "%s"
             ...
             ALGO_COLOR, s_timer_type[timer_type_], ALGO_COLOR, step_,
             ALGO_COLOR, s_event_type[event_type_], ALGO_COLOR, 0, ALGO_COLOR,
             s_var_select_type[0], ALGO_COLOR, "blah", ALGO_COLOR, "blah");
    wcscat(status_string, WANSI_COLOR_RESET);
}

void Timer::Start() { active_ = true; }
...
void Timer::SetDebug(bool b) { debug_ = b; }
